...
public class LinkedList<E> {
    ...
    public LinkedList<E> linearBinarySearch(String attribute, Object info) throws VacioException, IllegalAccessException, NonExistentElementException, IndexListException {
        LinkedList<E> result = new LinkedList<>();

        E element = binarySearch(attribute, info);

        if (element != null) {
            Integer index = indexOf(element);
            for (Integer i = index; i < getSize(); i++) {
                if (compareS(get(i), info) == 0) {
                    result.add(get(i));
                } else {
                    break;
                }
    ...
    private Integer indexOf(E element) throws VacioException, IndexListException, IllegalAccessException {
        for (Integer i = 0; i < getSize(); i++) {
            if (compareS(get(i), element) == 0) {
                return i;
            }
        }
        throw new IndexListException(size);
    }
